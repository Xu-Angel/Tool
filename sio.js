 ...
 * @param {Object} attrs 在script自定义添加的属性
 */
var _createScriptTag = function (scr, url, charset, attrs) {
  scr.setAttribute('type', 'text/javascript');
  charset && scr.setAttribute('charset', charset);
  scr.setAttribute('src', url);
  if (attrs) {
    for (attr in attrs) {
      scr.setAttribute(attr, attrs[attr]);
    }
  }
 ...
 */
var callByBrowser = function (url, opt_callback, opt_options, attrs) {
  ...
  _createScriptTag(scr, url, charset, attrs);
};
 ...
 * @meta standard
 */
var callByServer = function (url, callback, opt_options) {
  var scr = document.createElement('SCRIPT'),
    prefix = 'bd__cbs__',
    callbackName,
    callbackImpl,
